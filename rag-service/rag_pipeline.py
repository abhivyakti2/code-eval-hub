...
# why mmr? MMR (Maximal Marginal Relevance) is a retrieval method that aims to balance relevance and diversity in the retrieved documents. By using MMR, we can ensure that the retrieved commit diffs are not only relevant to the contributor's login but also diverse in terms of content. This is particularly important for generating evaluation questions, as we want to cover a range of topics and aspects of the contributor's work rather than just focusing on a narrow set of similar commits. MMR helps us achieve this by selecting documents that are both relevant to the query and different from each other, which can lead to more varied and insightful questions being generated by the language model.
# why not similarity search like other chains? Similarity search may retrieve documents that are very similar to each other, which could lead to generating questions that are focused on a narrow aspect of the contributor's work. By using MMR, we can ensure that we retrieve a more diverse set of commit diffs, which can help in generating a wider range of questions that cover different areas of the contributor's contributions. This can lead to a more comprehensive evaluation of the contributor's work and provide better insights for the interviewer.
